...
async function processUploadedDocument(io, code, params) {
  ...
  try {
    ...
    try {
      insight = await analyzeDocument(text, HF_TOKEN, francCode);
    } catch (analysisErr) {
      console.error("[doc pipeline] HF analysis failed (doc still usable for AI chat):", analysisErr.message || analysisErr);
    }
...
async function processUserDocument(io, code, email, params) {
  ...
  try {
    ...
    // Analyze doc via HF — if it fails, still store the doc so AI chat works
    let insight = { overview: "", keyPoints: "", deepDive: "", studyQuestions: "", objectives: "" };
    try {
      insight = await analyzeDocument(text, HF_TOKEN, francCode);
    } catch (analysisErr) {
